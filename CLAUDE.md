...
**The ECS task definition lives in the backend repo**, not here — `dupli1 infra/terraform/ecs_frontends.tf` → `aws_ecs_task_definition.manage_web` (family `dupli1-manage-web`, bridge/EC2, 256 CPU / 512 MB). Environment variables, CPU and memory are changed there and take effect on `terraform apply`. This repo's `.github/workflows/aws.yml` only builds the image, pushes `latest` (plus `<sha>` and `v<ver>-b<build>` for rollback) and calls `update-service --force-new-deployment` so ECS re-pulls it — the same shape the backend services use. It previously deployed its own `.aws/task-definition.json` under a second family and a FARGATE launch type, which meant two systems owned one service; that file is retired.
