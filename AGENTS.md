...
- Product prices and order totals display as **KRW only** (`STORE_CURRENCY` / `formatCurrency` / `formatKrw` in `app/lib/i18n`). Settings currency is locked to KRW (not a multi-currency picker).
